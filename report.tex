...
Mechanical Turk, or MTurk as it is commonly known, is a crowdsourcing Internet
marketplace where users can solve submitted tasks by other users. These tasks
are usually unable to be completed by computers. There are a large variety
of tasks which are submitted, such as identifying the artist of a particular
song and choosing the best photo for an album cover for a CD. The crowd, who
are known as \emph{Turkers}, browse through tasks and complete them for a
small payment which is set by the user who wants the task completed.

%also reference mechanical turk's website?
\begin{figure}
	\caption{MTurk's interface}
\end{figure}

\subsubsection{Microworkers}
As with MTurk, Microworkers is a crowdsourcing platform as well. They refer to
the crowd as \emph{Workers}, and the clients as \emph{Employers}. Tasks, or
\emph{micro jobs} as they are referred to, are assigned to the crowd and are 
paid for by the clients. Tasks include rating videos, writing reviews, visiting
websites, etc.

%also reference microworkers's website?
\begin{figure}
	\caption{Microworks's interface}
\end{figure}

\subsubsection{myLot}
myLot has been around for a while, but they only recently opened up their tasks
section, which was not around before. Before adding the tasks section, myLot
would pay its users for posting in discussions, commenting on blogs, etc.
myLot is particularly attractive to users as it provides multiple ways of earning
money.

%also reference myLot's website?
\begin{figure}
	\caption{myLot's interface}
\end{figure}
%%%%%%%%%%%
...
\begin{figure}
	\caption{Screenshot of our Github repository}
\end{figure}
